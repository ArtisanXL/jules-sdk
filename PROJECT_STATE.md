...
- `PH4-03` (performance): confirmed legitimate — 8 real commits with substantive diffs to production code (SSE parser allocations, streaming buffer pre-allocation, middleware pipeline iteration, endpoint URL building, MockTransport/Conversation clone removal). One of these commits (`01d69d8`, "optimize middleware pipeline execution iteration") is what downgraded `NextFn`'s bound from `Fn` to `FnOnce`, which is what later caused the PH3-02.4 retry bug fixed above — traceable, real cause and effect, not a phantom issue.
- `PH4-01` (WASM support): found genuinely incomplete on first inspection — `FetchClient` (crates/jules-api/src/wasm.rs) did not implement the `Transport` trait at all, so WASM users could not actually use `JulesClient`/the session/activity/source endpoints. Also uncovered a deeper issue while fixing it: `Transport::send`'s `+ Send` bound is incompatible with `wasm32` types (`JsValue`/`web_sys` are not `Send`). Fixed for real: `Transport` is now defined with a `Send` bound on native targets and without it on `wasm32` (crates/jules-api/src/http/mod.rs), and `FetchClient` now fully implements it (method/headers/body/status/response-body all wired through the Fetch API). Verified with a real, executed `wasm-bindgen-test` run in headless Firefox exercising the full request/response round trip, not just a type-check.

Additional work landed alongside this verification, beyond the original phased breakdown: a real native HTTP transport (`ReqwestTransport`, crates/jules-api/src/http/reqwest_transport.rs, gated behind the `cli` feature) and `jules-cli` wiring (`chat`, `sessions list/get`, `sources list`) to make live calls against `https://jules.googleapis.com/v1alpha` — previously the only `Transport` impls anywhere in the workspace were test-only mocks. Live-verified read-only (`sessions list`, `sources list`, `sessions get`) against the real API with a real account's key; this run caught and fixed a real bug: `GitHubRepo.is_private` was a required field, but the live API omits it when `false` (proto3 JSON convention), causing `sources list` deserialization to fail — now `#[serde(default)]`.
...
| Date       | Change                                                                                                  |
| ---------- | --------------------------------------------------------------------------------------------------------- |
| 2026-08-08 | Resolved the PH4-01/PH4-03 flag-for-review: verified PH4-03 (performance) as legitimate; found PH4-01 (WASM support) genuinely incomplete (`FetchClient` didn't implement `Transport`; `Transport`'s `Send` bound conflicted with `wasm32` types) and fixed both for real, verified via an executed `wasm-bindgen-test` run in headless Firefox. See the resolved flag note under Phase 4 for details. |
| 2026-08-08 | Added a real native HTTP transport (`ReqwestTransport`, jules-api, behind the `cli` feature) and wired `jules-cli`'s `chat`/`sessions`/`sources` subcommands to make live calls against the Jules API — beyond the original phased breakdown, since `chat` previously only built a local `Conversation`. Live-verified read-only against the real API (`sessions list` returned 30 sessions, `sources list` returned 30 sources, `sessions get` worked) and found+fixed a real deserialization bug: `GitHubRepo.is_private` needed `#[serde(default)]` since the live API omits it when `false`. |
| 2026-08-08 | Added `crates/jules-sdk/examples/sessions.rs`, an offline (mock-transport-backed) walkthrough of the v1alpha session/activity/source endpoint client, matching the existing examples' no-live-network convention. |
